...
NEWS_DAYS = 30  # Chỉ lấy tin trong 30 ngày gần nhất
...
BANK_NAMES = {
    ...
    "VPB": "VPBank",      "STB": "Sacombank",  "HDB": "HDBank",
    "TPB": "TPBank",      "VIB": "VIB",         "MSB": "MSB",
    "OCB": "OCB",         "SHB": "SHB",         "LPB": "LPBank",
    "EIB": "Eximbank",   "NAB": "Nam A Bank",  "PGB": "PG Bank",
    "ABB": "ABBank",     "BVB": "Bao Viet Bank",
}
ALIAS_MAP = {
    "VCB": ["vietcombank"],
    "BID": ["bidv"],
...
def backtest(ticker, pred_len, prices, scaler):
    ...
    for i in range(len(scaled_test)):
        if i + pred_len > len(scaled_test):
            break
        x_in = torch.tensor(
            test_series[i: i + INPUT_LEN], dtype=torch.float32
        ).unsqueeze(0)
        for name in MODEL_REGISTRY:
            m = load_model_cached(ticker, name, pred_len)
            if m is None:
...
def _parse_entry_date(entry) -> datetime | None:
    """Parse ngày từ RSS entry, trả về datetime timezone-aware (UTC)."""
...
def _fetch_feed_safe(url: str, timeout: int = 8) -> feedparser.FeedParserDict | None:
    """
    Tải RSS qua requests (kiểm soát timeout & encoding tốt hơn),
    sau đó parse bằng feedparser.
    Fallback: gọi feedparser trực tiếp nếu requests lỗi.
    """
    headers = {
        "User-Agent": (
            "Mozilla/5.0 (compatible; VNBankForecast/1.0)"
        ),
...
def _google_news_url(ticker: str, bank_name: str) -> str:
    """Tạo URL Google News RSS cho ngân hàng."""
    query = requests.utils.quote(f"{ticker} {bank_name} cổ phiếu ngân hàng")
    return f"https://news.google.com/rss/search?q={query}&hl=vi&gl=VN&ceid=VN:vi"


@st.cache_data(ttl=1800)
def fetch_news(ticker: str, max_results: int = 15) -> list[dict]:
    """
    Lấy tin tức liên quan đến ticker trong NEWS_DAYS ngày gần nhất.
    Nguồn: RSS chính (CafeF, VnExpress, Vietstock) + Google News fallback.
    """
    ...
    feed_urls = RSS_FEEDS + [_google_news_url(ticker, bank_name)]
    ...
    for feed_url in feed_urls:
        ...
        for entry in feed.entries:
            ...
            # Lọc theo từ khoá ngân hàng
            if not any(kw in combined for kw in keywords):
                continue

            # Lọc theo ngày — chỉ lấy tin trong NEWS_DAYS ngày
            ...
            pub_display = (
                pub_dt.astimezone().strftime("%d/%m/%Y %H:%M")
                if pub_dt else datetime.now().strftime("%d/%m/%Y")
            )
    ...
    collected.sort(key=lambda x: x["_pub_dt"], reverse=True)
    for item in collected:
        item.pop("_pub_dt", None)

    if not collected:
        collected = [{
            "title": "Không tìm thấy tin tức — thử lại sau",
            "url": "#",
            "source": "—",
            "published": datetime.now().strftime("%d/%m/%Y"),
            "summary": (
                f"Không tìm thấy tin tức cho {bank_name} ({ticker}) "
                f"trong {NEWS_DAYS} ngày gần nhất. "
                "Có thể do RSS feed tạm thời không khả dụng."
            ),
            "image": "",
        }]
...
if page == "📊 Tổng quan":
    ...
    for u, v, data in G.edges(data=True):
        ...
        color = (
            f"rgba(30,136,229,{alpha})"
            if val > 0
            else f"rgba(216,90,48,{alpha})"
        )
...
# ════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        with col_c:
            ...
            fig3 = go.Figure(go.Bar(
                x=sub["time"], y=sub["daily_return"], marker_color=colors_bar,
            ))
            fig3.update_layout(
                paper_bgcolor="#111827", plot_bgcolor="#111827",
                font_color="#e2e8f0", height=320, margin=dict(t=10, b=10),
        ...
        with col_d:
            ...
            fig4 = go.Figure(go.Bar(
                x=sub["time"], y=sub["volume"], marker_color="#60a5fa",
            ))
            fig4.update_layout(
                paper_bgcolor="#111827", plot_bgcolor="#111827",
                font_color="#e2e8f0", height=320, margin=dict(t=10, b=10),
    ...
    with tab2:
        ...
        for label, pred_len in PRED_LENS.items():
            ...
            price_split = int(len(prices) * 0.8)
            bt_dates = prices_dates[
                price_split: price_split + len(next(iter(bt_preds.values())))
            ]
        ...
        if not os.path.exists(EVAL_PATH):
            ...
            if df_ticker.empty:
                ...
                for pred_len, label in HORIZON_LABELS.items():
                    ...
                    df_display.columns = [
                        c.replace("model", "Mô hình").replace("VarianceRatio", "VR")
                        for c in available
                    ]
                    ...
                    if os.path.exists(mae_img):
                        st.image(mae_img, caption=f"MAE theo từng bước — horizon {pred_len} ngày")

                    wf_img = f"data/eval_plots/{ticker}_wf_pred{pred_len}.png"
                    if os.path.exists(wf_img):
                        st.image(wf_img, caption=f"Walk-forward CV (4 folds) — horizon {pred_len} ngày")
                    elif os.path.exists(WF_PATH):
                        wf_df = pd.read_csv(WF_PATH)
                        wf_t = wf_df[
                            (wf_df["ticker"] == ticker) & (wf_df["pred_len"] == pred_len)
                        ]
                        if not wf_t.empty:
                    ...
                    if os.path.exists(REGIME_PATH):
                        ...
                        reg_t = regime_df[
                            (regime_df["ticker"] == ticker) & (regime_df["pred_len"] == pred_len)
                        ]
                        if not reg_t.empty:
                            ...
                            if os.path.exists(regime_img):
                                st.image(regime_img, caption=f"MAE theo Regime — horizon {pred_len} ngày")

                    if os.path.exists(DM_PATH):
                        dm_df = pd.read_csv(DM_PATH)
                        dm_t = dm_df[
                            (dm_df["ticker"] == ticker) & (dm_df["pred_len"] == pred_len)
                        ]
                        if not dm_t.empty:
                            ...
                            dm_display = dm_t[[
                                "best_model", "DM_stat", "p_value",
                                "significant", "Naive_MAE", "Best_MAE",
                            ]].copy()
                            dm_display.columns = [
                                "Best model", "DM stat", "p-value",
...
# ════════════════════════════════════════════════════════════
    ...
    st.caption(
        f"Nguồn: CafeF · VnExpress · Vietstock · Google News (RSS) — "
        f"chỉ hiển thị tin trong **{NEWS_DAYS} ngày** gần nhất, cập nhật mỗi 30 phút"
    )
    ...
    bank_display = BANK_NAMES.get(ticker_news, ticker_news)
    date_from = (datetime.now() - timedelta(days=NEWS_DAYS)).strftime("%d/%m/%Y")
    date_to   = datetime.now().strftime("%d/%m/%Y")

    st.subheader(f"🏦 {bank_display} ({ticker_news})")
    st.caption(f"📅 Tin từ {date_from} đến {date_to}")

    with st.spinner("Đang tải tin tức từ các nguồn RSS..."):
        news_list = fetch_news(ticker_news)

    if not news_list:
        st.info("Chưa có tin tức nào trong khoảng thời gian này.")
    else:
